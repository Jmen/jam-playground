...
export function JamDetail({ initialJam }: { initialJam: Jam }) {
  const [jam, setJam] = useState<Jam>(initialJam);
  ...
  const refetchJam = async () => {
    const result = await api.jams.get(jam.id);
    if (result.data) {
      setJam(result.data);
    }
  };
  ...
  const handleCommitLoop = async (loopId: string, audioIds: string[]) => {
    ...
    const result = await api.loops.add(jam.id, {
  ...
  const handleMakePublic = async () => {
    ...
    const result = await api.jams.makePublic(jam.id);
    if (result.data) {
      await refetchJam();
    }

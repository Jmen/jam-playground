...
export function LoopPlayer({
  ...
  useEffect(() => {
    ...
    const trackGainNodes = trackGainNodesRef.current;
    const masterGainNode = masterGainNodeRef.current;

    return () => {
      stopAllAudio();
      unregisterStopFunction(loopId);
      trackGainNodes.forEach((gain) => gain.disconnect());
      if (masterGainNode) {
  ...
  return (
    <div
      className={cn(
        ...
        playingLoopId === loopId ? "border-primary" : "border-border",
      )}
    >
        ...
        <div className="flex space-x-2">
          {audioItems.map((audio) => (
            ...
            >
              <div
                className={cn(
                  "h-16 border-4 rounded-md flex items-center justify-center bg-card p-1 shadow-md overflow-hidden",
                  mutedTracks.get(audio.id) ? "border-muted" : "border-border",
                  soloedTracks.get(audio.id) && "border-primary",
                )}
              >
                {audio.url ? (
                  <Waveform
                    audioUrl={audio.url}
                    onReady={(ws) => handleWaveformReady(audio.id, ws)}
                  />
                ) : (
                  <div
              ...
              <div className="text-center text-xs text-gray-500 mt-1 h-4">
                {trackDurations.has(audio.id) &&
                  `${trackDurations.get(audio.id)?.toFixed(2)}s`}
              </div>
              <div className="flex-grow flex flex-col items-center justify-center pt-2">
                <div className="flex items-center h-24">
                  ...
                  <Slider
                    ...
                    step={0.01}
                    onValueChange={(value) =>
                      handleVolumeChange(audio.id, value)
                    }
